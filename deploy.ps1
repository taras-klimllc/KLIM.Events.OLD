...
function Invoke-DockerCompose {
    ...
    # Build argument list properly to avoid System.String[] conversion issues
    $argumentList = @("-f", $ComposeFile, "--env-file", $EnvFile)
    
    # Split command and add each argument individually to avoid nested arrays
    $commandArgs = $Command.Split(" ", [System.StringSplitOptions]::RemoveEmptyEntries)
    foreach ($arg in $commandArgs) {
        $argumentList += $arg
    }
    
    $process = Start-Process -FilePath "docker-compose" -ArgumentList $argumentList -Wait -PassThru -NoNewWindow
